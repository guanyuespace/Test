...
(function() {
  ...
  var H = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54],
    d = d.AES = p.extend({
      blD2x: function() {
        for (var a = this.J1x, c = a.words, d = a.sigBytes / 4, a = 4 * ((this.ctK9B = d + 6) + 1), e = this.ctJ9A = [], j = 0; j < a; j++)
          if (j < d) e[j] = c[j];
          else {
            var k = e[j - 1];
...
function biMultiply(a, b) {
  var d, h, i, k, c = new BigInt,
    ...
    f = biHighIndex(b);
  for (k = 0; f >= k; ++k) {
    for (var j = 0, d = 0, i = k; e >= j; ++j, ++i) h = c.digits[i] + a.digits[j] * b.digits[k] + d, c.digits[i] = h & maxDigitVal, d = h >>> biRadixBits;
    c.digits[k + e + 1] = d
  }
...
var checkifNullPlayLists = function(playlists, that) {
  ...
  if (!playlists || playlists.length == 0) {
    wx.showModal({
      title: 'userID',
      content: '该用户不存在or该用户下无歌单，是否重设userId',
      showCancel: true,
      cancelText: '否',
      cancelColor: '#ff0000',
      confirmText: '是',
      ...
      success: function(res) {
        ...
        if (res.confirm) {
          //重设UserId
          //navigateTo: 第二次重复显示该弹窗???   Tab页面 don't unLoad ...
          wx.redirectTo({
            url: '/pages/music/user/user',
            success: function(res) {
              console.log("重设UserId");
            },
            fail: function(res) {},
 ...
 * just toast here.
 */
var checkifNullMusics = function(musics, creator, that) {
  if (musics && musics.length != 0) {
    that.setData({
      ...
      //save in cache
      wx.setStorageSync("playlistid" + that.data.playlistid, that.data.playlistid);
      wx.setStorage({
        key: 'musics' + that.data.playlistid,
        data: musics,
        success: function(res) {
          console.log("save musics into cache");
        }
      });
      wx.setStorage({
        key: 'creator' + that.data.playlistid,
        data: creator,
        success: function(res) {
          console.log("save creator into cache");
        }
      })
    })
  } else {
 ...
 * 根据歌单id获取所有歌曲
 */
var getMusic = function(playlistid, that) {
  var req_str = "{\"id\":\"" + playlistid + "\",\"offset\":\"0\",\"total\":\"true\",\"limit\":\"1000\",\"n\":\"1000\",\"csrf_token\":\"\"}";
  var result = myFunc(req_str);
  wx.request({
    ...
    success: function(res) {
      ...
      if (res.statusCode == 200) {
        if (res.data && res.data.code == 200) {
          checkifNullMusics(res.data.playlist.tracks, res.data.playlist.creator, that);
        } else {
      ...
      if (res.statusCode == 200) {
        if (res.data && res.data.code == 200) {
          if (res.data.lrc && res.data.lrc.lyric) {
            ...
            /////翻译歌词--->  res.data.tlyric
            if (res.data.tlyric && res.data.tlyric.lyric)
              lyric_str += res.data.tlyric.lyric;
        ...
        if (res.data.code == 200) {
          that.setData({ //this.setData  undifined
            ...
            if (!that.data.advisedUsers || that.data.advisedUsers.length == 0) {
              ...
              console.log("users= " + JSON.stringify(that.data.advisedUsers));
            }
          })
        } else {
          console.log(res.data);
  ...
  wx.request({
    ...
    success: function(res) {
      ...
      if (res.data) {
        ...
        var text = res.data;
        var regex = /<meta\s+property=\"og:image\"\s+content=\"(.*)\"/
        result = text.match(regex)[1];
      }
      console.log("result= " + result);
      that.setData({
        avatarUrl: result
      }, () => {
        console.log("current avatar=" + that.data.avatarUrl);
        wx.setStorageSync("avatarUrl", that.data.avatarUrl);
        wx.setStorageSync("avatarUrl_id", uid);
      });
    },
    fail: function(res) {},
    complete: function(res) {},
